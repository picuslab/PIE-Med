...
def main():
    ...
    st.markdown("Welcome to the dashboard of the PIE-Med project!")

    desc = st.empty()
    desc1 = st.empty()
    desc.caption("Welcome to the dashboard for 🩺 **PIE-Med**, a cutting-edge system designed to enhance medical \
                    decision-making through the integration of Graph Neural Networks (GNNs) ⚙️, Explainable AI (XAI) ❓ \
    ...
    with l1:
        st.subheader("📋 Patient's complete medical history in the selected visit")
        ...
        st.subheader(f"🧾 Recommended/Predicted _{task}_ of the Last Visit")
        st.caption(f"""The following {task} are recommended/predicted for the patient during the last visit n°: \
                   {format(last_visit['visit_id'].item())}. \nThe recommendations/predictions are based on the \
        ...
        st.dataframe(list_output, column_config={"0": "ID", "1": f"Recommended/Predicted {task}"}, height=None, width=None)
    ...
    with r1:
        ...
        st.caption(f"""The following graph shows the explainability of the model's decision on the recommended/ \
                    predicted {task} for the patient during the visit n°: {format(visit)}. \n\n\nThe explainability 
                    is based on the **{algorithm} (_{task}_)** algorithm.""")
        options = [item[1] for item in list_output if item]
        selected_label = st.selectbox(f'Select the {task} to explain', index=None, 
                                        placeholder=f"Choice a {task} from Recommended/Predicted {task} ranking to \
                                            explain", options=options)
